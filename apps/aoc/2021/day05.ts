function parse(s: string): number[][][] {
  return s.trim().split('\n').map((v) => {return v.split('->').map((v) => {return v.trim().split(',').map(Number)})})
}

export function solve(input: string, boardsize: number, diagonals: boolean): number {
  let board = (new Array(boardsize)).fill(0).map((a) => { return new Array(boardsize).fill(0); });
