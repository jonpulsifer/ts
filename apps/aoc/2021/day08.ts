...
export function part2(input: string): number {
  ...
  for (const [p, o] of entries) {
    let digits: Map<number, string> = new Map;
    ...
    digits.set(3, patterns.find(v =>
      v.length === 5 && includes(v, digits.get(7) || '')
    ) || '');
    patterns.filter(v => v.length === 6).forEach((pattern) => { // 0, 6, 9
      if (includes(pattern, digits.get(3) || '')) {
        digits.set(9, pattern);
      } else if (includes(pattern, digits.get(7) || '') &&
    ...
    patterns.filter(v => v.length === 5).forEach((pattern) => { // 2, 3, 5
    ...
    const letters: Map<string, number> = new Map(Array.from(digits, a => [ a[1], a[0]]));
