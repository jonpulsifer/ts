...
export function useWeatherSocket() {
  ...
  const deviceIdRef = useRef<number>(Number.parseInt(import.meta.env.VITE_TEMPESTWX_DEVICE_ID || ""))
  ...
  const connect = useCallback((token: string, deviceId: number) => {
    ...
    try {
      const ws = new WebSocket(`wss://ws.weatherflow.com/swd/data?token=${token}`)
      wsRef.current = ws
      ...
      ws.onclose = (event) => {
        ...
        if (!isManualDisconnectRef.current && event.code !== 1000 && tokenRef.current && deviceIdRef.current) {
  ...
  useEffect(() => {
    if (tokenRef.current && deviceIdRef.current && connectionStatus === "disconnected" && !isManualDisconnectRef.current) {
